...
CalendarEvent.validate = data => {
  ...
  try {
    data._masterUUID = UTIL.inputValidation.validateString(data._masterUUID, '_masterUUID');
    data.uuid = UTIL.inputValidation.validateUUID(data.uuid);
    data.start = UTIL.inputValidation.validateTime(data.start, 'start');
    data.end = UTIL.inputValidation.validateTime(data.end, 'end');
    data.summary = UTIL.inputValidation.validateString(data.summary, 'summary');
    ...
    data.isRecurring = UTIL.inputValidation.validateBoolean(data.isRecurring, 'isRecurring');

    if (data.isRecurring) {
      data.humanRecurrence = UTIL.inputValidation.validateString(data.humanRecurrence, 'humanRecurrence');
      data._recurrenceRule = UTIL.inputValidation.validateString(data._recurrenceRule, '_recurrenceRule');
      data._noMore = UTIL.inputValidation.validateBoolean(data._noMore, '_noMore');
