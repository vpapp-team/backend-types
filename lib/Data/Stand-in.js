...
StandIn.validate = data => {
  ...
  try {
    ...
    if (data.type === 1) {
      ...
      data.lesson = UTIL.validateNullable(data.lesson, 'lesson', UTIL.validateLessonDiscrim);
      data.class = UTIL.validateNullable(data.class, 'class', UTIL.validateClassDiscrim);
