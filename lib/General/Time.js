...
Time.simpleNow = (hasTime = true) => `D${hasTime ? 'T' : ''}${hasTime ? Date.now() : Math.floor(Date.now() / DAY_MILISEC)}`; // eslint-disable-line max-len

Time.validate = data => {
  if (typeof data === 'string') {
    return _buildFromString(data.trim());
  } else if (typeof data === 'object') {
    return _buildFromObject(data);
  }
  return `unknown type "${typeof data}"`;
};
const _buildFromString = data => {
